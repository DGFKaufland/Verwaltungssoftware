...
public class User_Controller
{
    ...
    public void createStore ( final Users user )
    {
    	...
    	System.out.println(user.getMobile_user_role());
    	
    	if (user.getMobile_user_role().contains("ja")){
    		dbs_users.createMobileUser(user);   		
    		Users mobile_contact_user = dbs_users.selectMobileUser(user);    		
    		Integer ID = mobile_contact_user.getMobile_contact_id();
    		user.setMobile_contact_id(ID); 		
    	}
    	else{
    		user.setMobile_contact_id(0);
    	}
    ...
    {
    	if (user.getMobile_user_role().contains("ja")){
    		dbs_users.updateMobileContactUser(user);  		
    	}    	
    ...
    public void deleteSensordata (final Users user)
    {
    	...
    	Users check = dbs_users.selectUser(user);
    	
    	if (check.getMobile_user_role().contains("ja")){
    		dbs_users.deleteMobileUser(check);
