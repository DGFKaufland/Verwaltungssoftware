...
public class DBStatements_DeparturesStores extends DBVerbindung
{
    ...
    public final Departures_to_store selectDepartmentByID(Departures_to_store dep)
    {

    	Departures_to_store dep_output = new Departures_to_store();
    	
        Connection con = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        String query = "SELECT ID, Abteilungstyp_ID, Markt_ID, Warenbereichsleiter_id, Server "
        		+ "FROM Abteilung abt WHERE ID = ?";
        ...
        try
        {
            ...
            while ( rs.next() )
            {         	
                dep_output.setID(rs.getInt("ID"));
                dep_output.setDeparture_type_id(rs.getInt("Abteilungstyp_ID"));
                dep_output.setStore_id(rs.getInt("Markt_ID"));
                dep_output.setWbl_id(rs.getInt("Warenbereichsleiter_id"));
                dep_output.setServer(rs.getString("Server"));
            }
        ...
        {
        	log.error( "Fehler beim Laden von getDepartureByID aus der Datenbank!", e );
            //log.error( "Fehler beim Laden aller Regeln!", e );
        }
        finally
        {
